...
def verifica_letra(caracter):
    return caracter not in string.punctuation and caracter != ' '

def verifica_sortudo(numero):
    return '2' in str(numero) and '7' not in str(numero)

# Ponteirnulovile
def verifica_digitos(digito):
    soma = 0
    for i, numero in enumerate(digito):
        if i > 0:
...
def imprime_mes_por_extenso(data):
    ...
    for i,m  in enumerate(meses):
        if mes == i:
            data_extenso = ('%s de %s de %s' %(dia, meses[i], ano))
    return data_extenso
...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    '''
    qtd_sortudos = 0
    for limite_inferior in range(limite_inferior, limite_superior):
        if verifica_sortudo(limite_inferior):
            qtd_sortudos += 1
    return qtd_sortudos
...
def ponteironuloville(telefones):
    ...
    for i, numero in enumerate(telefones):
        if verifica_digitos(telefones[i]):
